...
def evaluate_record(candidate_books, llm_output, threshold=7):
    """
...
with open("llm_ranking_without_description_with_all_users.json", "r", encoding="utf-8") as f:
    records = json.load(f)

per_model_metrics = defaultdict(lambda: {"ndcg": [], "ndcg_5": [], "ndcg_10": [], "rho": [], "rr": []})

for record in records:
    candidate_books = record.get("candidate books", [])
    ndcg_5, ndcg_10, ndcg_all, rho, rr = evaluate_record(candidate_books, record["llm_output"])
